...
# === CONFIG: DANZHIT HANLAI + ONEWEB ===
HEIR_ID = "John Danzhit Carroll, Doyon #D-456789"
LAND_DESC = "Danzhit Hanlai Trail, Yukon Flats, AA-12345"
HUGHES_API_KEY = "your_hughes_api_key"  # From OneWeb reseller (Hughes/Intellian)
...
# === LOGGING ===
logging.basicConfig(filename='fpt_oneweb.log', level=logging.INFO)
...
# === ONEWEB TERMINAL METRICS (Hughes/Intellian API) ===
...
class FPTOneWebBot:
    def __init__(self):
        self.log_file = "fpt_oneweb_receipts.jsonl"
        self.pdf_dir = "fpt_oneweb_pdfs"
    ...
    def create_receipt(self, metrics, veto=VETO_DEFAULT):
        ...
        # Encrypt
        key = HKDF(algorithm=hashes.SHA3_256(), length=32, salt=None, info=b"fpt_oneweb").derive(os.urandom(32))
    ...
    def save_pdf(self, r):
        ...
        pdf.cell(200, 10, txt="FPT + ONEWEB RECEIPT — DANZHIT HANLAI", ln=1, align='C')
...
def main():
    bot = FPTOneWebBot()
    print("FPT + ONEWEB — DANZHIT HANLAI LOOP ACTIVE")
    while True:
        m = get_oneweb_metrics()
        if m and m['gps']:
