...
# === LOGGING ===
logging.basicConfig(filename='fpt_starlink.log', level=logging.INFO)
...
def get_starlink_metrics():
    try:
        r = requests.get(f"https://api.starlink.com/v1/dish/{DISH_ID}/metrics", 
                        headers={"Authorization": f"Bearer {STARLINK_API_KEY}"}, timeout=10)
...
# === FPT RECEIPT BOT ===
class FPTStarlinkBot:
    def __init__(self):
        self.log_file = "fpt_starlink_receipts.jsonl"
        self.pdf_dir = "fpt_starlink_pdfs"
    ...
    def create_receipt(self, metrics, veto=VETO_DEFAULT):
        ...
        receipt = {
            ...
            "dish_id": DISH_ID, "gps": metrics.get('gps', {}),
        ...
        # Encrypt
        key = HKDF(algorithm=hashes.SHA3_256(), length=32, salt=None, info=b"fpt_starlink").derive(os.urandom(32))
    ...
    def save_pdf(self, r):
        ...
        pdf.cell(200, 10, txt="FPT + STARLINK RECEIPT — DANZHIT HANLAI", ln=1, align='C')
...
def main():
    bot = FPTStarlinkBot()
    print("FPT + STARLINK — DANZHIT HANLAI LOOP ACTIVE")
    while True:
        m = get_starlink_metrics()
        if m and m['gps']:
