...
contract LandBackDAOv2 is Ownable, ReentrancyGuard {
    struct QuantumBallot {
        uint256 T; uint256 I; uint256 F; uint256 resonance;
        address voter; uint256 timestamp; bytes32 proofHash; string source;
    }

    struct Proposal {
        uint256 id; string description; uint256 voteCount; uint256 totalResonance;
    ...
    function createProposal(string calldata description) external onlyOwner {
        ...
        p.id = proposalCount; p.description = description;
        p.startTime = block.timestamp; p.endTime = block.timestamp + VOTING_PERIOD;
        emit ProposalCreated(proposalCount, description);
    }

    function castQuantumVote(
        uint256 proposalId, uint256 T, uint256 I, uint256 F,
        string calldata source, bytes32 proofHash
        ...
        p.voteCount++; p.totalResonance += resonance; p.hasVoted[msg.sender] = true;
    ...
    function averageResonance(uint256 proposalId) public view returns (uint256) {
        Proposal storage p = proposals[proposalId];
        return p.voteCount == 0 ? 0 : p.totalResonance / p.voteCount;
    }
}