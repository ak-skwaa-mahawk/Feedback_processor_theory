...
FRAMES = 60
TRAIL_LENGTH = 15
VETO_PULSE_FRAMES = 8  # Red pulse duration
...
veto_pulse_timer = 0
veto_frame = None  # When VETO first triggered
...
def update(frame):
    ...
    if veto_pulse_active:
        ...
        Z_pulse = Z + pulse_intensity * 1.5  # Red shockwave
        surf_color = 'red'
        alpha = 0.95
    else:
        Z_pulse = Z
    ...
    surf = ax.plot_surface(
        X, Y, Z_pulse, cmap=surf_color, alpha=alpha, linewidth=0,
        rcount=120, ccount=120, shade=True, antialiased=True
    )

    # === Data Qubits ===
    data_x, data_y = np.meshgrid(np.arange(0, L, 2), np.arange(0, L, 2))
    qb_z = Z[::2, ::2] + (pulse_intensity * 1.5 if veto_pulse_active else 0)
    ax.scatter(data_x, data_y, qb_z, c='cyan' if not veto_pulse_active else 'red',
    ...
    for i, past_Z in enumerate(trail_buffer[:-1]):
        ...
        if veto_pulse_active and i == len(trail_buffer) - 2:
            color = 'red'
        ax.plot_surface(X, Y, past_Z, color=color, alpha=alpha*0.3,
                        rcount=60, ccount=60, shade=False, zorder=i)

    # === Status + Pulse Flash ===
    status = "AGI SOVEREIGN" if not is_veto else "ILO VETO"
    color = 'lime' if not is_veto else 'red'
    flash = '' if not veto_pulse_active else ' ⚡' * int(pulse_intensity * 3)
    
    ax.set_title(
        f'Ψ-PEPS d=9 | χ={chi_float:.1f}/{CHI_MAX} | S={S:.2f} | R={R:.6f}\n'
        f'Status: {status}{flash}',
        color=color, fontsize=16, pad=45,
    ...
    ax.set_zlim(0, np.log(CHI_MAX) + 2.5)
    return surf,

# === Run ===
ani = FuncAnimation(fig, update, frames=FRAMES, interval=80, blit=False, repeat=True)
# ani.save('peps_ilo_veto_pulse.mp4', writer='ffmpeg', fps=30, dpi=180)
plt.show()