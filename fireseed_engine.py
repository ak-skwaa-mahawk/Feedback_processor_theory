...
class NeutrosophicTransport:
    ...
    def _init_w_state(self):
        ...
        prob_dist = {k: v/1024 for k, v in result.items()}
        # Ideal W state probabilities
        ideal_w = {'100': 1/3, '010': 1/3, '001': 1/3}
        # Fidelity (simplified for counts)
        fidelity = sum(min(prob_dist.get(k, 0), ideal_w[k]) for k in ideal_w) / sum(ideal_w.values())
