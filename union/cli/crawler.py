...
@click.option('--interval', default=60.0, type=float, help='Repair daemon interval (default: 60s)')
@click.option('--once', is_flag=True, help='Repair once and exit')
def crawler(root, sway_mode, scapegoat, repair, repair_all, interval, once):
    ...
    if repair or repair_all:
        ...
        if once:
            click.echo(f"🔧 One-time sovereign repair on {path}")
            rmp.repairfile(path)
        else:
            click.echo(f"🔧 Starting sovereign repair daemon on {path} (interval {interval}s)")
            rmp.startcoderepair_daemon(path, interval=interval)
