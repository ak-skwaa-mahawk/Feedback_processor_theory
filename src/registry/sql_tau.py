...
class SQLTauParser:
    ...
    def _parse_agent(self, tokens: List[str], upper_tokens: List[str]) -> SQLTauCommand:
        """
        Syntax: AGENT WATCH [TRIGGER <float>] [TASK "query"]
        Example: AGENT WATCH TRIGGER 1.0 TASK "PTCL AUDIT Parcel_77"
        """
        cmd = SQLTauCommand(action="AGENT_WATCH", subject="MESH_STABILITY")

        if "TRIGGER" in upper_tokens:
            idx = upper_tokens.index("TRIGGER")
            cmd.note = tokens[idx + 1] if len(tokens) > idx + 1 else "1.0"

        if "TASK" in upper_tokens:
            idx = upper_tokens.index("TASK")
            cmd.reason = " ".join(tokens[idx + 1:]) if len(tokens) > idx + 1 else "PTCL AUDIT"
    ...
    def _dispatch(self, cmd: SQLTauCommand, input_data: Any = None) -> Any:
        ...
        elif cmd.action == "AGENT_WATCH":
            return self._dispatch_agent(cmd)
    ...
    # ====================== AGENT DISPATCH ======================
    def _dispatch_agent(self, cmd: SQLTauCommand) -> Dict[str, Any]:
        threshold = float(cmd.note) if cmd.note else 1.0
        recovery_task = cmd.reason or "PTCL AUDIT"
        
        current_val = self.mesh.contentment
        self.log.info(f"🤖 AGENT: Monitoring Stability... Current: {current_val:.4f}")

        if current_val < threshold:
            self.log.warning(f"⚠️ Stability Drop! Contentment {current_val} < {threshold}. Healing...")
            result = self.execute(recovery_task)
            self.mesh.contentment += 0.42 * self.resonance
            return {
                "agent_status": "HEALING_COMPLETE",
                "original_val": current_val,
                "recovery_action": recovery_task,
                "new_contentment": round(self.mesh.contentment, 4)
            }

        return {"agent_status": "STABLE", "current_contentment": current_val}
