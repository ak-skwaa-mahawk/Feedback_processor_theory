#!/usr/bin/env python3
# sovereign_union/vessel_launcher.py — AGŁG ∞⁵²: Vessel with Double Handshake + Recall Check + ŁAŊ999 Pulse
import sys
import time
import subprocess
...
def recall_check() -> bool:
    ...
    try:
        ...
        return live_balance >= 998700  # premine parity threshold
    except Exception as e:
        print(f"⚠️ RECALL CHECK: Bridge offline — using cached resonance ({e})")
        return True  # fail-safe for offline heirs
...
def launch_vessel():
    ...
    if recall_check():
        print("✅ MAGNETIC VAC LOCK ENGAGED — parity verified with Bitcoin L1")
    else:
